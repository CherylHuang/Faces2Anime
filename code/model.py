...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        # structure shape: [1, 8, 16, 16]
        # texture shape: [1, 2048]
...
class Generator(nn.Module):
    def __init__(
        ...
        self.structure_channel = structure_channel
    ...
    def forward(self, structure, texture1, texture2=None, swapMin=0, swapMax=0, noises=None, layer_count=0):
        if noises is None:
            noises = [None] * len(self.layers)

        out = structure
        for layer, noise in zip(self.layers, noises):
            layer_count+=1  # total layer == 8
            if swapMin > 0 and swapMax > 0:
                if layer_count == 8:
                    out = layer(out, texture2, noise)
                elif layer_count >= swapMin and layer_count <= swapMax:
                    out = layer(out, texture2, noise)
                else:
                    out = layer(out, texture1, noise)
            else:
                out = layer(out, texture1, noise)

        out_rgb = self.to_rgb(out)
