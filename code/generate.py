...
if __name__ == "__main__":
    ...
    for imgpath in args.files[: len(args.files) // 2 * 2]:
        ...
        img = img_open.convert("RGB").resize((imgsize, imgsize))
        img_a = (
        ...
        imgs.append(img_a)
        # store file name
        img_name = img_open.filename.split('/')[-1].split('.')[0]
    ...
    with torch.no_grad():
        ...
        #for i in range(7):
        #    if i > 0:
        #        out_swap = gen(struct1, texture1, texture2, i, 8)
        #        swap_imgs.append(out_swap)

        swapMin = 6
        swapMax = 8
        out_swap = gen(struct1, texture1, texture2, swapMin, swapMax)
        #print(out_swap.shape)
        #print(out.shape)

    font = ImageFont.truetype(
        "Px437_IBM_VGA_8x16.ttf", 24
    )
    ...
    reconsA_name = args.outpath + "recons_" + imgs_name[0] + ".png"
    reconsB_name = args.outpath + "recons_" + imgs_name[1] + ".png"
    swapAB_name = args.outpath + "swap_" + imgs_name[0] + "_" + imgs_name[1] + ".png"
    swapBA_name = args.outpath + "swap_" + imgs_name[1] + "_" + imgs_name[0] + ".png"
    #utils.save_image(out1, reconsA_name, nrow=1, normalize=True, range=(-1, 1))
    #utils.save_image(out2, reconsB_name, nrow=1, normalize=True, range=(-1, 1))
    #utils.save_image(out12, swapAB_name, nrow=1, normalize=True, range=(-1, 1))
    #utils.save_image(out21, swapBA_name, nrow=1, normalize=True, range=(-1, 1))

    #for img, i in zip(swap_imgs, range(len(swap_imgs))):
    #    swapL_name = args.outpath + "swap" + str(i) + "8_" + imgs_name[1] + "_" + imgs_name[0] + ".png"
    #    utils.save_image(img, swapL_name, nrow=1, normalize=True, range=(-1, 1))

    swapL_name = args.outpath + "swap" + str(swapMin) + str(swapMax) + "_" + imgs_name[1] + "_" + imgs_name[0] + ".png"
    utils.save_image(out_swap, swapL_name, nrow=1, normalize=True, range=(-1, 1))